...
`─── <group> ───` divider row (`ATTD_NOOP`) separates the categories.
...
| 📈 Attendance Rate | `ATTD_RATE` / `ATTD_RATE_<n>` | per-member regular-training attendance rate = ATTENDANCE col B `Tabulation` ÷ polled-REG-date count. Active members only. Each row `🟢 *100%*  (SU3) Wei Yin  (12/15)` (🟢≥80 / 🟡≥50 / 🔴 dot, `%` clamped at 100). Sorted **Seniors → Juniors → unknown**, then year category (Graduate → Exchange → Postgrad yr desc → Undergrad yr desc), then rate desc, then nickname; a **blank line** separates each category group. Paginated **20/page**. `_member_tag_map()` builds the `(S/J)(G/EG/P<n>/U<n>)` tag + sort key from MEMBER INFO `Seniority` (col Q) + `Year`. Data from `get_training_attendance_rates()` → `(nick, attended, polls, tag)` (smart-cached, 0 API calls warm) |
